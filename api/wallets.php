...
<section>
    <form>
        ...
        <fieldset><legend>Send</legend>
            ...
            <input type="checkbox" name="mine[state]"> propose (min 5 000 sats) <input type="integer" name="mine[amount]" min="5000" value="5000"> LSats with <select name="mine[utxo]" multiple></select> following <select name="mine[template]"></select> to <select name="mine[address]" multiple></select> for mine (each)<br>
            <input type="checkbox" name="pegIn[state]"> propose (min 5 000 sats) <input type="integer" name="pegIn[amount]" min="5000" value="5000"> Sats with <select name="pegIn[utxo]" multiple></select> following <select name="pegIn[template]"></select> to <select name="pegIn[address]" multiple></select> for peg in (each)<br>
            <input type="checkbox" name="pegOut[state]"> propose (min 10 000 sats) <input type="integer" name="pegOut[amount]" min="10000" value="10000"> Sats with <select name="pegOut[utxo]" multiple></select> following <select name="pegOut[template]"></select> to <select name="pegOut[address]" multiple></select> for for peg out (each)<br>
            <input type="checkbox" name="share[state]"> propose (min 10 000 sats) <input type="integer" name="share[amount]" min="10000" value="100000"> LSats with <select name="share[utxo]" multiple></select> following <select name="share[template]"></select> to <select name="share[address]" multiple></select> for share <input type="file" name="tempalte">  (each)<br>
            <input type="checkbox" name="backup[state]"> propose (min 1 000 sats) <input type="integer" name="backup[amount]" min="1000" value="1000"> LSats with <select name="backup[utxo]" multiple></select> following <select name="backup[template]"></select> to <select name="backup[address]" multiple></select> for backup (each)<br>
            <input type="checkbox" name="lock[state]"> propose (min 10 000 sats) <input type="integer" name="lock[amount]" min="10000" value="10000"> LSats with <select name="lock[utxo]" multiple></select> following <select name="lock[template]"></select> to <select name="lock[address]" multiple></select> for lock (each)<br>
            <input type="checkbox" name="witness[state]"> propose (min 10 sats) <input type="integer" name="witness[amount]" min="10" value="10"> LSats with <select name="witness[utxo]" multiple></select> following <select name="witness[template]"></select> to <select name="witness[address]" multiple></select> for witness (each)<br>
            <input type="checkbox" name="node[state]"> propose (min 10 sats) <input type="integer" name="node[amount]" min="10" value="10"> LSats with <select name="node[utxo]" multiple></select> following <select name="node[template]"></select> to <select name="node[address]" multiple></select> for processing (each nodes)<br>
            <p>By the way for destination:</p>
            <input type="checkbox" name="proposal_template[state]"> propose (min 10 sats) <input type="integer" name="proposal_template[amount]" min="10" value="10"> LSats with <select name="proposal_template[utxo]" multiple></select> following <select name="proposal_template[template]"></select> for add <select name="proposal_template[proposal]"></select> template<br>
            <input type="checkbox" name="proposal_share[state]"> propose (min 10 000 sats) <input type="integer" name="proposal_share[amount]" min="10000" value="10000"> LSats with <select name="proposal_share[utxo]" multiple></select> following <select name="proposal_share[template]"></select> for add <select name="proposal_share[proposal]"></select> node for sharing<br>
            <input type="checkbox" name="proposal_backup[state]"> propose (min 1 000 sats) <input type="integer" name="proposal_backup[amount]" min="1000" value="1000"> LSats with <select name="proposal_backup[utxo]" multiple></select> following <select name="proposal_backup[template]"></select> for add <select name="proposal_backup[proposal]"></select> address for backup<br>
            <input type="checkbox" name="proposal_lock[state]"> propose (min 10 000 sats) <input type="integer" name="proposal_lock[amount]" min="10000" value="10000"> LSats with <select name="proposal_lock[utxo]" multiple></select> following <select name="proposal_lock[template]"></select> for add <select name="proposal_lock[proposal]"></select> address for lock<br>
            <input type="checkbox" name="proposal_witness[state]"> propose (min 10 sats) <input type="integer" name="proposal_witness[amount]" min="10" value="10"> LSats with <select name="proposal_witness[utxo]" multiple></select> following <select name="proposal_witness[template]"></select> for add <select name="proposal_witness[proposal]"></select> address for witness<br>
            <input type="checkbox" name="proposal_node[state]"> propose (min 10 sats) <input type="integer" name="proposal_node[amount]" min="10" value="10"> LSats with <select name="proposal_node[utxo]" multiple></select> following <select name="proposal_node[template]"></select> for add <select name="proposal_node[proposal]"></select> node for processing<br>
            <p></p>
            <input type="submit" name="transaction" value="Send"><br>
        </fieldset>
        <fieldset><legend>Convert</legend>
            <input type="integer" name="convertSat[amount]"> Sats with <select name="convertSat[utxo]" multiple></select> following <select name="convertSat[template]"></select> to <select name="convertSat[address]" multiple></select> for convert into LSAT<br>
            <input type="integer" name="convertLSat[amount]"> LSats with <select name="convertLSAT[utxo]" multiple></select> following <select name="convertLSat[template]"></select> to <select name="convertLSAT[address]" multiple></select> for convert into SAT<br>
            <input type="submit" name="conversion" value="Send"><br>
        </fieldset>
        <fieldset><legend>Send a template</legend>
            <input type="integer" name="convertSat[amount]"> Sats with <select name="convertSat[utxo]" multiple></select> following <select name="convertSat[template]"></select> to <select name="convertSat[address]" multiple></select> for convert into LSAT<br>
            <input type="integer" name="convertLSat[amount]"> LSats with <select name="convertLSAT[utxo]" multiple></select> following <select name="convertLSat[template]"></select> to <select name="convertLSAT[address]" multiple></select> for convert into SAT<br>
            <input type="submit" name="conversion" value="Send"><br>
        </fieldset>
